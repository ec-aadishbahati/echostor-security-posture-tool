...
def test_generate_standard_report(client: TestClient, auth_token, completed_assessment):
    with patch("app.api.reports.BackgroundTasks.add_task") as mock_task:
...
def test_generate_report_incomplete_assessment(client: TestClient, auth_token, test_assessment):
    response = client.post(
...
def test_request_ai_report(client: TestClient, auth_token, completed_assessment):
    with patch("app.api.reports.BackgroundTasks.add_task") as mock_task:
        response = client.post(
            ...
            headers={"Authorization": f"Bearer {auth_token}"},
            json={"assessment_id": str(completed_assessment.id), "message": "Please generate AI report"}
        )
        assert response.status_code == 200
        data = response.json()
...
def test_get_user_reports(client: TestClient, auth_token, test_report):
    response = client.get("/api/reports/user/reports", headers={
...
def test_admin_generate_ai_report(client: TestClient, admin_token, db_session, completed_assessment):
...
def test_admin_generate_ai_report_unauthorized(client: TestClient, auth_token, test_report):
    response = client.post(

...
def test_start_assessment_new(client: TestClient, test_user, auth_token):
    response = client.post("/api/assessment/start", headers={
...
def test_start_assessment_existing(client: TestClient, test_user, auth_token, test_assessment):
    response = client.post("/api/assessment/start", headers={
        "Authorization": f"Bearer {auth_token}"
    })
    assert response.status_code == 200
...
def test_get_current_assessment(client: TestClient, auth_token, test_assessment):
    response = client.get("/api/assessment/current", headers={
        "Authorization": f"Bearer {auth_token}"
    })
    assert response.status_code == 200
    data = response.json()
...
def test_get_assessment_responses(client: TestClient, auth_token, test_assessment, test_assessment_response):
    response = client.get(f"/api/assessment/{test_assessment.id}/responses", headers={
        "Authorization": f"Bearer {auth_token}"
    })
    assert response.status_code == 200
    data = response.json()
...
def test_get_assessment_responses_not_found(client: TestClient, auth_token):
    response = client.get("/api/assessment/nonexistent-id/responses", headers={
        "Authorization": f"Bearer {auth_token}"
    })
    assert response.status_code == 404
...
def test_save_assessment_progress_update_existing(client: TestClient, auth_token, test_assessment, test_assessment_response):
    response = client.post(
        f"/api/assessment/{test_assessment.id}/save-progress",
        headers={"Authorization": f"Bearer {auth_token}"},
        json={
...
def test_save_consultation_interest_invalid_word_count(client: TestClient, auth_token, test_assessment):
    ...
    response = client.post(
        ...
        json={
            "consultation_interest": True,
            "consultation_details": short_details
        }
    )

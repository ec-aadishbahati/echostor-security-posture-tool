...
def test_get_user_assessments(client: TestClient, admin_token, test_user, test_assessment):
    response = client.get(
...
def test_get_all_assessments(client: TestClient, admin_token, test_assessment):
    response = client.get("/api/admin/assessments", headers={
        "Authorization": f"Bearer {admin_token}"
    })
    assert response.status_code == 200
    data = response.json()
...
def test_get_all_assessments_with_status_filter(client: TestClient, admin_token, test_assessment):
    response = client.get(
...
def test_get_dashboard_stats(client: TestClient, admin_token, test_user, test_assessment):
...
def test_get_users_progress_summary(client: TestClient, admin_token, test_user):
    response = client.get("/api/admin/users-progress-summary", headers={
        "Authorization": f"Bearer {admin_token}"
    })
    assert response.status_code == 200
    data = response.json()
...
def test_get_consultation_requests(client: TestClient, admin_token, db_session, test_user):
    ...
    response = client.get("/api/admin/consultations", headers={
        "Authorization": f"Bearer {admin_token}"
    })
    assert response.status_code == 200
    data = response.json()

...
def test_complete_user_flow(client: TestClient):
    ...
    start_response = client.post("/api/assessment/start", headers={
...
def test_admin_workflow(client: TestClient, admin_token, test_user, admin_completed_assessment):
    users_response = client.get("/api/admin/users", headers={
        "Authorization": f"Bearer {admin_token}"
    })
    assert users_response.status_code == 200
    
    assessments_response = client.get("/api/admin/assessments", headers={
        "Authorization": f"Bearer {admin_token}"
    })
    assert assessments_response.status_code == 200
    
    stats_response = client.get("/api/admin/dashboard/stats", headers={
        "Authorization": f"Bearer {admin_token}"
    })

...
class TestPIIRedactionBeforeAI:
    ...
    def test_pii_redacted_in_answers_before_ai(self, mock_settings, mock_key_manager, mock_openai):
        ...
        mock_response.choices[0].message.content = '{"summary": "test", "key_findings": [], "recommendations": [], "gaps": []}'
        ...
        with patch('app.services.report_generator.AICacheService') as mock_cache_service:
            ...
            with patch('app.services.report_generator.benchmark_context_service') as mock_benchmark:
                mock_benchmark.get_relevant_context.return_value = []
                
                with patch('app.services.report_generator.build_section_prompt_v2') as mock_build_prompt:
                    mock_build_prompt.return_value = ("test prompt", 0)
                    
                    with patch('app.services.report_generator.safe_validate_section_artifact') as mock_validate:
    ...
    @patch('app.services.report_generator.OpenAIKeyManager')
    @patch('app.services.report_generator.settings')
    def test_pii_redacted_in_comments_before_ai(self, mock_settings, mock_key_manager, mock_openai):
    ...
    @patch('app.services.report_generator.OpenAIKeyManager')
    @patch('app.services.report_generator.settings')
    def test_pii_redaction_metrics_tracked(self, mock_settings, mock_key_manager, mock_openai, mock_metrics):

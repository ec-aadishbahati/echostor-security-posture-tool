...
def update_admin_user():
    ...
    try:
        ...
        admin_user = db.query(User).filter(User.email == 'aadish.bahati@echostor.com').first()
        ...
        if admin_user:
            ...
            updated_user = db.query(User).filter(User.email == 'aadish.bahati@echostor.com').first()
            print(f"✅ Verification: {updated_user.email} is_admin = {updated_user.is_admin}")
        else:

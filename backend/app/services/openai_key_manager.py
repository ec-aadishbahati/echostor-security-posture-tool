...
class OpenAIKeyManager:
    ...
    def get_next_key(self) -> tuple[str, str]:
        ...
        query = (
            ...
            .filter(
                and_(
                    ...
                    (OpenAIAPIKey.cooldown_until.is_(None)) | (OpenAIAPIKey.cooldown_until <= now)
                )
            )
            .order_by(
        ...
        decrypted_key = decrypt_api_key(key.encrypted_key)
        logger.info(f"Selected API key: {key.key_name} (ID: {key.id}, usage: {key.usage_count})")
    ...
    def toggle_key(self, key_id: str, is_active: bool) -> OpenAIAPIKey:
        ...
        logger.info(f"Toggled key {key.key_name} to {'active' if is_active else 'inactive'}")
        return key

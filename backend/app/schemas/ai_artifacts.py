...
class SectionAIArtifact(BaseModel):
    ...
    @classmethod
    def validate_recommendations_have_signals(cls, recs: list[Recommendation]) -> list[Recommendation]:
        for rec in recs:
    ...
    def validate_risk_matches_gaps(cls, risk_level: str, info) -> str:
        if 'gaps' in info.data:
            critical_gaps = sum(1 for g in info.data['gaps'] if g.severity == 'Critical')
            if critical_gaps > 0 and risk_level not in ['High', 'Critical']:
                raise ValueError("Risk level must be High/Critical when critical gaps exist")
        return risk_level

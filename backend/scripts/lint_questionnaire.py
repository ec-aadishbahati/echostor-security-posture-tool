...
class QuestionnaireIssue:
    ...
    def __init__(self, line_num: int, issue_type: str, description: str, context: str = ""):
...
def detect_placeholder_options(lines: List[str]) -> List[QuestionnaireIssue]:
    ...
    generic_desc_patterns = [
        re.compile(r'\*Basic Description:\*\s+(First|Second|Third|Fourth|Fifth)\s+option'),
    ...
    for i, line in enumerate(lines, start=1):
        if option_pattern.search(line):
            issues.append(QuestionnaireIssue(
                i, "PLACEHOLDER_OPTION", 
...
def detect_content_mismatches(lines: List[str]) -> List[QuestionnaireIssue]:
    ...
    sast_dast_pattern = re.compile(r'(SAST|DAST|static\s+analysis|dynamic\s+analysis)', re.IGNORECASE)
    ...
    for i, line in enumerate(lines, start=1):
        ...
        if sast_dast_pattern.search(line):
            if "Application Security" not in current_section and "DevSecOps" not in current_section:
...
def main():
    ...
    if not questionnaire_path.exists():
        print(f"ERROR: Questionnaire file not found: {questionnaire_path}", file=sys.stderr)
        sys.exit(1)
